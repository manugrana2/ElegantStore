        ...
        <section class="collection">
            <div class="container">
                <h2 class="collection__title">
                    <span class="font-bold title__line__left">Lo </span> <span
                        class="font-bold text-primary">Último</span>
                </h2>
                ...
                <div class="row mt-4">
                    ...
                    <?php
                        ...
                        $get_shoes = $database->getRows("SELECT * FROM products ORDER BY `product_name` ASC");
                        ...
                        for ($i=0; $i <count($get_shoes) ; $i++) { 
                            ...
                            if (isset($_SESSION['userId'])) {
                                 $check_cart = $database->getRow("SELECT * FROM john_cart WHERE product_id = ? AND user_id = ?",[$get_shoes[$i]['product_id'],$_SESSION['userId']]);
                                if ($check_cart) {
                                    $style = 'block';
                                }else{
                                    $style = 'none';
                           ...
                           echo'
                                <div class="col-md-3 mb-4 d-flex">
                                    <div class="card shoe__card">
                                        <img class="shoe__image" src="assets/images/store/'.$get_shoes[$i]['product_image_1'].'" alt="">
                                        <div class="shoe__card__overlay">
                                            ...
                                            <span class="overlay__icons">
                                                <button class="btn btn-icon btn-card-icon" data-shoe-id="'.$get_shoes[$i]['product_id'].'">
                                                    <img class="cart__icon" width="100%" src="assets/images/icons/shopping-bag.svg"
                                                        alt="">
                                                </button>
                                            </span>
                                            <span class="overlay__info">
                                                <h6 class="overlay__title">'.$get_shoes[$i]['product_name'].'</h6>
                                                <small class="overlay__text">'.$get_shoes[$i]['product_name'].'</small>
                                                <h6 class="overlay__price">$ <span class="shoe__card__item__price">'.$get_shoes[$i]['product_price'].'</span>
                                                </h6>
                                            </span>
                                        </div>
                                    </div>
