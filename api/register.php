<?php
    ...
    $check_user_existence = $database->getRow("SELECT * FROM users WHERE user_email=?",[$email]);
    ...
    if ($user_exists) {
        ...
        $register_user = $database->insertRow("INSERT INTO users (firstname, lastname, user_email, `user_password`) VALUE(?,?,?,?)",[$firstname, $lastname,$email,$password]);
